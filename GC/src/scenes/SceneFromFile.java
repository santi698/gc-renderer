			...
			if(inCamera){
				...
				if(inLight){
					if(currentLine.startsWith("Transform")){
						...
						lightTransform = new Matrix4d(	Double.parseDouble(values[0]),Double.parseDouble(values[4]),Double.parseDouble(values[8]),Double.parseDouble(values[12]),
														Double.parseDouble(values[1]),Double.parseDouble(values[5]),Double.parseDouble(values[9]),Double.parseDouble(values[13]),
														Double.parseDouble(values[2]),Double.parseDouble(values[6]),Double.parseDouble(values[10]),Double.parseDouble(values[14]),
														Double.parseDouble(values[3]),Double.parseDouble(values[7]),Double.parseDouble(values[11]),Double.parseDouble(values[15]));
	...
	public Light getLightFromArguments(String lightType, Map<String,Object> arguments, Matrix4d lightTransform){
		...
		switch(lightType){
			case "point":
				...
				light = new PointLight(from, color, intensity, lightTransform);
			...
			case "infinite":
				...
				light = new AmbientLight(color2, intensity);
	...
	public void geometryParser(String fileDir) throws FileNotFoundException{
		...
		while(in.hasNextLine()){
			...
			if(currentLine.startsWith("Transform")){
				...
				transform = new Matrix4d(	Double.parseDouble(values[0]),Double.parseDouble(values[4]),Double.parseDouble(values[8]),Double.parseDouble(values[12]),
												Double.parseDouble(values[1]),Double.parseDouble(values[5]),Double.parseDouble(values[9]),Double.parseDouble(values[13]),
												Double.parseDouble(values[2]),Double.parseDouble(values[6]),Double.parseDouble(values[10]),Double.parseDouble(values[14]),
												Double.parseDouble(values[3]),Double.parseDouble(values[7]),Double.parseDouble(values[11]),Double.parseDouble(values[15]));
